...
function loadImages(sources) {
  return Promise.all(sources.map(it => {
    const i = new Image();
    i.src = it;
...
loadImages([
  'static/OmusubiKororin-1.png',
  'static/OmusubiKororin-2.png',
  'static/OmusubiKororin-3.png',
  'static/OmusubiKororin-4.png',
  'static/OmusubiKororin-5.png',
  'static/OmusubiKororin-6.png',
  'static/OmusubiKororin-7.png'
]).then((images) => {
  const foo = new headbreaker.Canvas('foo-canvas', {
    width: 1500,
    height: 5000,
    pieceSize: 200,
    proximity: 40,
  ...
  foo.autogenerate({
    ...
    insertsGenerator: headbreaker.sequence.random,
    metadata: [
      {image: {content: images[1], scale: ((8/6) * 200  / images[1].width), offset: {x: 120, y: 120}}},
      {image: {content: images[2], scale: ((8/6) * 200 / images[2].width), offset: {x: 120, y: 120}}},
      {label: {text: 'había una vez un anciano\nmuy trabajador', y: 0, strokeColor: 'white', fontSize: 20}},
      {label: {text: 'hatarakimono no ojiisan ga\n imashita', y: 0, strokeColor: 'white', fontSize: 20}},
      {image: {content: images[3], scale: ((8/6) * 200 / images[3].width), offset: {x: 120, y: 120}}},
      {image: {content: images[4], scale: ((8/6) * 200 / images[4].width), offset: {x: 120, y: 120}}},
      {label: {text: 'el anciano escuchó unas voces\n que cantaban\n desde dentro del agujero ', y: 0, strokeColor: 'white', fontSize: 20}},
      {label: {text: 'ojiisan ga ana wo nozoku to\n naka kara utagoe ga\n kikoetekimashita', y: 0, strokeColor: 'white', fontSize: 20}},
      {image: {content: images[5], scale: ((8/6) * 200 / images[5].width), offset: {x: 120, y: 120}}},
      {image: {content: images[6], scale: ((8/6) * 200 / images[6].width), offset: {x: 120, y: 120}}},
      {label: {text: 'esp', y: 0, strokeColor: 'white'}},
      {label: {text: 'hira', y: 0, strokeColor: 'white'}},
    ]
  });
  //foo.shuffle(0.7);
  foo.draw();
