...
const sl = (p: Piece, t: number[], s: number[], n: number[]): number[] =>
  select(p.left, t, s, n);
const sr = (p: Piece, t: number[], s: number[], n: number[]): number[] =>
  select(p.right, t, s, n);
const su = (p: Piece, t: number[], s: number[], n: number[]): number[] =>
  select(p.up, t, s, n);
const sd = (p: Piece, t: number[], s: number[], n: number[]): number[] =>
  select(p.down, t, s, n);
...
export class Squared {
  ...
  draw(
    ...
    return [
      ...
      (it, index) =>
        (Number(it) * (index % 2 === 0 ? sizeVector.x : sizeVector.y)) / 5
    );
  }
...
export class Rounded {
  ...
  draw(
    ...
    return [
      ...
      0,
      r2sy - ny(b1),
      ...(b1 ? /** bezel: */ [0, r2sy, 0, r2sy, b.x, r2sy] : /**        */ []),
      /** edge:   */ nx(b1),
      r2sy,
      s.x,
      r2sy,
...
export default { Squared, Rounded, Classic };
